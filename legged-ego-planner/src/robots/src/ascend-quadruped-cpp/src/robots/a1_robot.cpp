...
namespace Quadruped {
A1Robot::A1Robot(std::string configFilePath)
{
...
//    standUpMotorAngles << defaultStandUpAngle, defaultStandUpAngle, defaultStandUpAngle, defaultStandUpAngle;
    ...
    std::cout << "-------A1Robot init Complete-------" << std::endl;
}

void A1Robot::ReceiveObservation()
{
...
void A1Robot::ReceiveObservationTest(LowState state)
{
    std::array<float, 4> quaternion = state.imu.quaternion;
    std::array<float, 3> rpy = state.imu.rpy;
    std::array<float, 3> gyro = state.imu.gyroscope;
    ...
    std::array<int16_t, 4> force = state.footForce;
    footForce << force[0], force[1], force[2], force[3];
    for (int footId; footId < numLegs; footId++) {
        if (footForce[footId] > 20) {
            footContact[footId] = true;
        } else {
            footContact[footId] = false;
...
void A1Robot::ApplyAction(const Eigen::MatrixXf &motorCommands, MotorMode motorControlMode)
{
...
void A1Robot::ApplyAction(const std::vector<MotorCommand> &motorCommands, MotorMode motorControlMode)
{
//    std::cout << "apply:\n" << motorCommands << std::endl;
    std::array<float, 60> motorCommandsArray = {0};
    for (int motorId = 0; motorId < numMotors; motorId++) {
...
std::array<float, 60> A1Robot::ApplyActionTest(const Eigen::MatrixXf &motorCommands, MotorMode motorControlMode)
{
    std::array<float, 60> motorCommandsArray = {0};
    if (motorControlMode == POSITION_MODE) {
        Eigen::Matrix<float, 1, 12> motorCommandsShaped = motorCommands;
        for (int motorId = 0; motorId < numMotors; motorId++) {
            motorCommandsArray[motorId * 5] = motorCommandsShaped[motorId];
            motorCommandsArray[motorId * 5 + 1] = motorKps[motorId];
            motorCommandsArray[motorId * 5 + 2] = 0;
            motorCommandsArray[motorId * 5 + 3] = motorKps[motorId];
            motorCommandsArray[motorId * 5 + 4] = 0;
        }
    } else if (motorControlMode == TORQUE_MODE) {
        Eigen::Matrix<float, 1, 12> motorCommandsShaped = motorCommands;
    ...
    return motorCommandsArray;
}

void A1Robot::Step(const Eigen::MatrixXf &action, MotorMode motorControlMode)
{
    ReceiveObservation();
